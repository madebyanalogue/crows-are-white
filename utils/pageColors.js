...
function srgbUnitToLinear(unit) {
  if (unit <= 0.04045) return unit / 12.92
  return ((unit + 0.055) / 1.055) ** 2.4
}

function linearToSrgbUnit(linear) {
  if (linear <= 0.0031308) return linear * 12.92
  return (1.055 * (linear ** (1 / 2.4))) - 0.055
}

function formatDisplayP3Channel(value) {
  return String(Math.round(Math.min(1, Math.max(0, value)) * 1000) / 1000)
}

/** Convert sRGB hex to a CSS display-p3 color(). */
export function hexToDisplayP3Color(hex) {
  const channels = parseHexChannels(hex)
  if (!channels) return null

  const linearRed = srgbUnitToLinear(channels.r / 255)
  const linearGreen = srgbUnitToLinear(channels.g / 255)
  const linearBlue = srgbUnitToLinear(channels.b / 255)

  const displayRed = linearToSrgbUnit(
    (0.6274039147 * linearRed) + (0.3292830345 * linearGreen) + (0.0433130188 * linearBlue),
  )
  const displayGreen = linearToSrgbUnit(
    (0.0690972546 * linearRed) + (0.9195408735 * linearGreen) + (0.0113613960 * linearBlue),
  )
  const displayBlue = linearToSrgbUnit(
    (0.0163914479 * linearRed) + (0.0880132892 * linearGreen) + (0.8955957409 * linearBlue),
  )

  return `color(display-p3 ${formatDisplayP3Channel(displayRed)} ${formatDisplayP3Channel(displayGreen)} ${formatDisplayP3Channel(displayBlue)})`
}

/** Approximate sRGB hex fallback for a display-p3 color() string. */
export function displayP3ColorToHex(value) {
  const match = String(value).match(/color\(\s*display-p3\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)/i)
  if (!match) return null

  const linearRed = srgbUnitToLinear(Number(match[1]))
  const linearGreen = srgbUnitToLinear(Number(match[2]))
  const linearBlue = srgbUnitToLinear(Number(match[3]))

  const srgbLinearRed = (1.224940176 * linearRed) + (-0.224940176 * linearGreen)
  const srgbLinearGreen = (-0.042056943 * linearRed) + (1.042056943 * linearGreen)
  const srgbLinearBlue = (-0.019637554 * linearRed) + (-0.078636046 * linearGreen) + (1.098273604 * linearBlue)

  const toHex = (linear) => Math.round(linearToSrgbUnit(linear) * 255)
    .toString(16)
    .padStart(2, '0')

  return `#${toHex(srgbLinearRed)}${toHex(srgbLinearGreen)}${toHex(srgbLinearBlue)}`
}

function normalizeWideGamutColorInput(value) {
  if (typeof value === 'string') return value.trim()
  return normalizeColorValue(value)
}

/**
 * Parse CMS colour input into sRGB + optional display-p3 values.
 * Supports brand tokens, hex, display-p3 strings, and `#hex|color(display-p3 …)` pairs.
 */
export function parseWideGamutColorInput(value) {
  const normalized = normalizeWideGamutColorInput(value)
  ...
  if (normalized.includes('|')) {
    const [srgbPart, p3Part] = normalized.split('|').map((part) => part.trim())
    if (!srgbPart || !p3Part) return null

    const p3 = p3Part.startsWith('color(')
      ? p3Part
      : `color(display-p3 ${p3Part})`
  ...
  if (normalized.startsWith('color(display-p3')) {
    return {
      srgb: displayP3ColorToHex(normalized) || normalized,
      p3: normalized,
      isToken: false,
    }
  }
  ...
  if (normalized.startsWith('#')) {
    return {
      srgb: normalized,
      p3: hexToDisplayP3Color(normalized),
      isToken: false,
    }
  }
...
/** Build inline CSS custom properties for wide-gamut colours. */
export function getWideGamutStyleVars(value, fallbackToken, cssVarName) {
  const parsed = parseWideGamutColorInput(value)
  if (!parsed) {
    return {
      [cssVarName]: toCssColor(undefined, fallbackToken),
    }
  }

  if (parsed.isToken) {
    return {
      [cssVarName]: parsed.srgb,
    }
  }
...
export function isLightColor(color) {
  let normalized = normalizeColorValue(color)
