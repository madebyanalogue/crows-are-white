...
const background = computed(() => {
  ...
  const videoUrl = mediaType === 'video'
    ? resolveSanityAssetUrl(props.section?.newsletterVideo?.asset)
    : null
  ...
  return {
    ...
    // Flips the scrim to black so light copy stays readable over the media.
...
// Only set colours the editor actually chose, so the section inherits the page palette otherwise.
